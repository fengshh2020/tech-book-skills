...
快速模式不建立全书知识库，不执行全书冲突映射，不跑全书覆盖率：
...
仍需在 `progress.md` 记录跳过条件、源书范围、目标章节、未做全量映射的风险。
...
4. 从主书选取 3-5 段代表性段落，记录风格基线。风格基线需要记录：平均句长、术语密度、叙述节奏（先概念后示例 vs 先示例后概念）、技术深度预期。
